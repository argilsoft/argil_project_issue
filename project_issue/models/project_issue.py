    ...
    def _compute_day(self):
        for issue in self:
            ...
            if issue.date_closed:
                ...
                issue.working_hours_close = calendar.get_work_hours_count(dt_create_date, dt_date_closed, False,
                    compute_leaves=True) #, resource_id=False, default_interval=(8, 16))
    ...
    def create(self, vals):
        if vals.get('code', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['code'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('project.issue') or _('New')
            else:
                vals['code'] = self.env['ir.sequence'].next_by_code('project.issue') or _('New')
